...
class Master extends Frontend
{
    ...
    public function tmplate_list(){
        if(request()->isAjax()){
            ...
            $part = request()->get('release_type',0);
            if(!empty($name)){
                $name = trim($name,' ');
            ...
            if(!empty($release_info)){
                foreach($release_info as $k=>$v){
                    ...
                    $release_info[$k]['is_status1'] = Db::table('yb_attitude')->where(['release_id'=>$v['id'],'user_id'=>$uid,'status'=>1])->count();
                    $release_info[$k]['is_status2'] = Db::table('yb_attitude')->where(['release_id'=>$v['id'],'user_id'=>$uid,'status'=>2])->count();
                    $release_info[$k]['is_status3'] = Db::table('yb_attitude')->where(['release_id'=>$v['id'],'user_id'=>$uid,'status'=>3])->count();
                    $release_info[$k]['is_status4'] = Db::table('yb_attitude')->where(['release_id'=>$v['id'],'user_id'=>$uid,'status'=>4])->count();
                    $release_info[$k]['is_status5'] = Db::table('yb_attitude')->where(['release_id'=>$v['id'],'user_id'=>$uid,'status'=>6])->count();
                    //获取盒子是否添加
                    if(!empty($box)){
                    ...
                    //json转数组
                    $release_info[$k]['effect'] = json_decode($v['effect'],true);
                    $release_info[$k]['category'] = json_decode($v['category'],true);
                }
            }
            ...
            $this->view->assign('release_type',$part);
     ...
     * 获取详情信息
     */
    public function get_detadil($andwhere,$state=0){
        $user = new users();
        $business = new company();
        $uid = $this->uid;
        //获取样板信息
        ...
        if(!empty($release_detail['reference'])){
            $rcount = count($release_detail['reference']);
            $rcount = ceil($rcount/2);
            $release_detail['reference'] = array_chunk($release_detail['reference'],$rcount);
            $release_detail['i'] = 0;
        }
        //添加浏览记录
    ...
    public function tmplate_detail(){
        if(request()->isAjax()){
            ...
            $uid = $this->uid;
            $andwhere['part'] = 1;
            $this->get_detadil($andwhere);
            //获取用户评论
            $comment = new comment();
            $comment_list = $comment->get_comment(['a.status'=>0,'m.level'=>0,'m.type'=>0]);
            foreach($comment_list as $ck=>$c){
                ...
                $comment_list[$ck]['reply'] = $comment->get_comment(['a.status'=>0,'m.pid'=>$c['id']]);
            }
    ...
    public function material_detail(){
        if(request()->isAjax()){
            ...
            $andwhere['part'] = request()->get('release_type',0);
            $this->get_detadil($andwhere,1);
            return $this->view->fetch();
        }
    }
    /**
    ...
    public function attitude(){
        ...
        if($status){
            $result = Db::table('yb_attitude')->where(['id'=>$status['id']])->delete();
            ...
            $result = Db::table('yb_attitude')->insert($data);
